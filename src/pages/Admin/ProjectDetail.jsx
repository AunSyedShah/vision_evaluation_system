...
const AdminProjectDetail = () => {
  ...
  if (!project) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/admin/projects" className="text-indigo-600 hover:text-indigo-700 mt-4 inline-block">
          Back to Projects
        </Link>
      </div>
    );
  ...
  return (
    <div>
      <div className="mb-8">
        <Link to="/admin/projects" className="text-indigo-600 hover:text-indigo-700 mb-4 inline-block">
          ← Back to Projects
        </Link>
      ...
      <div className="bg-white rounded-xl shadow-md mb-6">
        ...
        <div className="p-6">
          ...
          {activeTab === 'evaluators' && (
            <div>
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Assigned Evaluators</h3>
                <p className="text-sm text-gray-600">View evaluators assigned to this project</p>
              </div>

              {!project.assignedEvaluators || project.assignedEvaluators.length === 0 ? (
                <div className="text-center py-8 bg-gray-50 rounded-lg">
                  <p className="text-gray-600">No evaluators assigned yet</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {project.assignedEvaluators.map((evaluatorId) => {
                    const evaluator = getEvaluatorById(evaluatorId);
                    return evaluator ? (
                      <div key={evaluatorId} className="flex items-center justify-between p-4 border-2 border-gray-200 rounded-lg">
                        <div>
